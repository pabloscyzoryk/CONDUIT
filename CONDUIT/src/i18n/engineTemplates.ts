...
export const ENGINE_TEMPLATES: readonly (readonly [string, string])[] = [
  ...
  ["T-100: liczba kontekstów w kwarantannie po luce w obserwacji: {n}; historia oczekujących zdarzeń zachowana, nowe potwierdzone sygnały nadal mogą być używane", "T-100: {n} contexts quarantined after an observation gap; pending history retained, new confirmed signals remain usable"],
  ["T-100: bierna kolejka kontekstu utraciła {n} zdarzeń; dotknięte luką stare konteksty wymagają nowej pełnej wersji źródła", "T-100: passive context queue lost {n} events; affected old contexts require a new complete source version"],
  ["Nie można przywrócić aktywnego trybu strategii rachunku: {e}", "Cannot restore the account's active strategy mode: {e}"],
  ["nieprawidłowe dzienne kotwice rachunku (invalid_account_day_anchors)", "invalid_account_day_anchors"],
  ["nieaktualne dzienne kotwice rachunku (stale_account_day_anchors)", "stale_account_day_anchors"],
  ["M1: nieprawidłowe potwierdzenie strumienia", "M1 feed acknowledgement is invalid"],
  ["M1: połączenie zmieniło się podczas subskrypcji", "M1 connection changed during subscription"],
  ["Tryb zmienił się od wysłania polecenia; wybierz tryb ponownie", "The mode changed after the request was sent; select the mode again"],
  ["Zmiana trybu wymaga potwierdzonego pustego rachunku i uzgodnienia wszystkich zleceń; dotychczasowa ochrona pozostaje aktywna", "Changing mode requires a confirmed flat account and reconciliation of all orders; existing protection remains active"],
  ["Zmiana trybu czeka na potwierdzone połączenie i pusty rachunek MT5", "Changing mode awaits a confirmed connection and a flat MT5 account"],
  ["Zmiana trybu wymaga aktualnego identyfikatora sesji rachunku (accountSession)", "Changing mode requires the current account session identifier (accountSession)"],
  ["Zmiana trybu pochodzi ze starej sesji FOLLOW", "The mode-change request belongs to an old FOLLOW session"],
  ["M1: zmieniono konfigurację strumienia", "M1 feed configuration changed"],
  ["M1: oczekiwanie na bieżące połączenie", "M1 waiting for current connection"],
  ["M1: strumień nie jest gotowy dla bieżącego połączenia", "M1 feed not ready for current connection"],
  ["M1: niezgodny schemat albo rachunek/instrument", "M1 schema or account/symbol mismatch"],
  ["M1: niepełne dane", "M1 incomplete"],
  ["M1: nieprawidłowe dane albo świeca nie jest zamknięta", "M1 malformed or not closed"],
  ["M1: ograniczone uzupełnienie; pominięto starsze niedostępne świece", "M1 bounded catchup: older unavailable candles were omitted"],
  ["M1: tożsamość rachunku niepotwierdzona albo zmieniona", "M1 account identity unconfirmed or changed"],
  ["M1: nieprawidłowa ramka transportu", "M1 malformed wire frame"],
  ["M1: przepełnienie kolejki dostaw; wymagane ponowne zebranie historii", "M1 delivery queue overflow; warmup recovery required"],
  ...
  ["nieobsługiwany kontrakt brokera lub waluta rachunku", "unsupported broker contract or account currency"],
  ["T-100 WERYFIKACJA: {reason}; nowe wejścia zablokowane, ochrona pozostaje aktywna", "T-100 REVIEW: {reason}; new exposure blocked, protection remains active"],
  ...
  ["T-100 WEJŚCIE: decyzja {} potwierdzona w koszyku {}", "T-100 ENTRY: decision {} confirmed in basket {}"],
  ["T-100 M1: {reason}; nowe wejścia czekają na pełne świece", "T-100 M1: {reason}; new entries await complete candles"],
  ["brak zapisu stanu T100", "missing T100 checkpoint"],
  ["nieobsługiwana wersja zapisu stanu T100", "unsupported T100 checkpoint revision"],
  ["konfiguracja zapisu stanu T100 jest inna", "T100 checkpoint configuration differs"],
  ["zamknięta transakcja nie ma zweryfikowanej podstawy netto", "closed receipt has no validated net basis"],
  ["potwierdzone wejście nie ma zgodnej migawki pozycji", "acknowledged entry has no matching position snapshot"],
  ["parametry potwierdzonego wejścia są nieprawidłowe", "confirmed entry geometry is invalid"],
  ["identyfikator autonomicznego koszyka jest już zajęty", "autonomous basket identity is already occupied"],
  ["nieprawidłowa konfiguracja T100", "invalid T100 configuration"],
  ["konfiguracja zmieniła się bez zgodnego stanu decyzji", "configuration changed without matching decision state"],
  ["przesunięcie UTC brokera nie jest jawną obsługiwaną liczbą pełnych godzin", "broker UTC offset is not an explicit supported whole hour"],
  ["wyczerpano numerację koszyków", "basket sequence exhausted"],
  ["wyczerpano numerację koszyków w slocie", "basket slot exhausted"],
  ["wysłane wejście nie ma dokładnego identyfikatora potwierdzenia", "submitted entry has no exact confirmation identity"],
  ["Ustawienia T-100: oczekiwano obiektu", "T-100 settings: expected an object"],
  ["Ustawienia T-100: nieznane pole {key}", "T-100 settings: unknown field {key}"],
  ["Ustawienia T-100: nieprawidłowe typy pól", "T-100 settings: invalid field types"],
  ["Ustawienia T-100: wartości poza dozwolonym zakresem", "T-100 settings: values outside the allowed range"],

  ["BRAK PAKIETU MetaTrader5. Zainstaluj: pip install MetaTrader5", "MetaTrader5 PACKAGE MISSING. Install: pip install MetaTrader5"],
  ["Uwaga: pakiet jest 64-bitowy i TYLKO pod Windows.", "Note: the package is 64-bit and Windows-only."],
  ["BRAK PAKIETU MetaTrader5. Zainstaluj: pip install MetaTrader5 | Uwaga: pakiet jest 64-bitowy i TYLKO pod Windows.{detail}", "MetaTrader5 PACKAGE MISSING. Install: pip install MetaTrader5 | Note: the package is 64-bit and Windows-only.{detail}"],
  ["MT5: start nieudany [{stage}], kod {code}: {e}", "MT5: startup failed [{stage}], code {code}: {e}"],
  ["nie można potwierdzić bieżącej sesji Windows", "cannot confirm the current Windows session"],
  ["nieczytelna lista procesów MT5", "unreadable MT5 process list"],
  ["nie można potwierdzić sesji procesu MT5; wybór terminala wstrzymany", "cannot confirm the MT5 process session; terminal selection is on hold"],
  ["nie można odczytać ścieżki terminala w tej sesji Windows; sprawdź uprawnienia MT5 i bota", "cannot read the terminal path in this Windows session; check MT5 and bot permissions"],
  ["ścieżka MT5 musi wskazywać pełny plik terminal64.exe", "the MT5 path must be an absolute path to terminal64.exe"],
  ["wybrany terminal nie jest uruchomiony w tej sesji Windows; połączenie zostanie ponowione", "the selected terminal is not running in this Windows session; the connection will be retried"],
  ["MT5 nie jest uruchomiony w tej sesji Windows; połączenie zostanie ponowione", "MT5 is not running in this Windows session; the connection will be retried"],
  ["działa więcej niż jeden terminal MT5; wskaż jednoznaczną ścieżkę albo zamknij dodatkową instancję", "more than one MT5 terminal is running; specify an unambiguous path or close the extra instance"],
  ["follow-terminal wymaga Windows", "follow-terminal requires Windows"],
  ["nie można odczytać listy procesów Windows", "cannot read the Windows process list"],
  ["lista procesów Windows jest niepełna", "the Windows process list is incomplete"],
  ["nie udało się uruchomić procesu Pythona: {e}", "could not start the Python process: {e}"],
  ["proces Pythona zakończył się przed połączeniem z botem; diagnostyka: {e}", "the Python process exited before connecting to the bot; diagnostics: {e}"],
  ["brak potwierdzenia gotowości terminala w czasie inicjalizacji", "terminal readiness was not confirmed during initialization"],
  ["sidecar zakończył połączenie przed potwierdzeniem gotowości terminala", "the sidecar closed the connection before terminal readiness was confirmed"],
  ["oczekiwanie na gotowość terminala przerwane: {e}", "waiting for terminal readiness was interrupted: {e}"],
  ["wersja protokołu sidecara {received} zamiast {expected}", "sidecar protocol version {received} instead of {expected}"],
  ["sidecar nie potwierdził gotowości MT5; starszy protokół nie podał szczegółów startu", "the sidecar did not confirm MT5 readiness; the older protocol supplied no startup details"],
  ["nieczytelny opis błędu inicjalizacji sidecara", "unreadable sidecar initialization error details"],
  ["sidecar MT5 nie podłączył się w ciągu 120 s", "the MT5 sidecar did not connect within 120 s"],
  ["initialize() nieudane: {code} {detail}", "initialize() failed: {code} {detail}"],
  ["błąd inicjalizacji sidecara: {exception}", "sidecar initialization error: {exception}"],
  ["terminal nie ma potwierdzonego zalogowanego konta", "the terminal has no confirmed logged-in account"],
  ["niejednoznaczny/brak pełnohandlowego XAUUSD lub XAUUSD.s dla tego brokera", "a fully tradable XAUUSD or XAUUSD.s symbol is missing or ambiguous for this broker"],
  ["konto terminala zmienione; wymagana nowa sesja bota", "the terminal account changed; a new bot session is required"],
  ["konto zmienione; rejestr potwierdzeń wymaga nowej sesji", "the account changed; the execution confirmation ledger requires a new session"],
  ["symbol {symbol} niedostępny w Podglądzie rynku", "symbol {symbol} is unavailable in Market Watch"],
  ["brak symbol_info dla {symbol}", "no symbol_info for {symbol}"],
  ["stara sesja po zmianie konta; polecenie odrzucone", "stale session after an account change; command rejected"],
  ["login/serwer/typ konta zmieniony; polecenie odrzucone", "login/server/account type changed; command rejected"],
  ["follow: konto niepotwierdzone", "follow: account unconfirmed"],
  ["follow: terminal jest na rachunku REAL/CONTEST; handel zablokowany. DEMO jest domyślne; REAL wymaga jawnego mt5_allow_real_account=true", "follow: the terminal is on a REAL/CONTEST account; trading is blocked. DEMO is the default; REAL requires explicit mt5_allow_real_account=true"],
  ["nie udało się pobrać parametrów {symbol}: {e}", "could not retrieve parameters for {symbol}: {e}"],
  ["BŁĄD STARTU [{stage}]: {e}", "STARTUP ERROR [{stage}]: {e}"],
  ["Historia rachunku z terminala", "Terminal account history"],
  ["Brak ukończonego eksportu allLogs", "No completed allLogs export is available"],
  ["Niedozwolona ścieżka eksportu allLogs", "Unsafe allLogs export path"],
  ["Niedozwolona ścieżka pobierania allLogs", "Unsafe allLogs download path"],
  ["Można pobrać wyłącznie ukończony eksport allLogs", "Only the completed allLogs export can be downloaded"],
  ["Niedozwolony katalog wynikowy allLogs", "Unsafe allLogs output directory"],
  ["Eksport allLogs znajduje się poza katalogiem wynikowym", "allLogs export is outside its output directory"],
  ["Nieprawidłowa nazwa eksportu allLogs", "Invalid allLogs export name"],
  ["Nie można otworzyć ukończonego eksportu allLogs", "Cannot open the completed allLogs export"],
  ["Nie można sprawdzić ukończonego eksportu allLogs", "Cannot inspect the completed allLogs export"],
  ["Przekroczono limit rozmiaru pobierania allLogs", "allLogs download byte limit exceeded"],
  ["Brak ścieżki eksportu allLogs", "Missing allLogs export path"],
  ["Proces pobierania allLogs zakończył się błędem", "allLogs download worker failed"],
  ["REARM REVIEW: brak pełnego dowodu wysłanej intencji; wymagane uzgodnienie historii brokera i pamięci koszyka", "REARM REVIEW: complete submission evidence is missing; broker history and basket memory require reconciliation"],
  ["REARM HOLD: oczekiwanie na potwierdzenie wysłanego zlecenia; licznik nie jest zgadywany", "REARM HOLD: waiting for confirmation of the submitted order; the counter is not inferred"],
  ["REARM REVIEW: stan licznika nie odpowiada zapisanej próbie; wymagane uzgodnienie pamięci koszyka", "REARM REVIEW: counter state does not match the recorded attempt; basket memory requires reconciliation"],
  ["REARM potwierdzony po uzgodnieniu z brokerem: przezbrojenie {count}; zachowano czas wysłania", "REARM confirmed after broker reconciliation: rearm {count}; original submission time retained"],
  ["REARM: potwierdzone wcześniej wysłane zlecenie; bez ponownej wysyłki", "REARM: previously submitted order confirmed; no resubmission"],
  ["REARM REVIEW: brak potwierdzonej pozycji o pełnej tożsamości wysłanego zlecenia; wymagane uzgodnienie historii brokera", "REARM REVIEW: no confirmed position with the complete submitted order identity; broker history requires reconciliation"],
  ["Nagranie odtwarzania live niedostępne", "Replay capture unavailable"],
  ["Nagranie odtwarzania live niekompletne", "Replay capture incomplete"],
  ["Zapis nagrania nie powiódł się: {e}", "capture write failed: {e}"],
  ["Nie udało się utrwalić nagrania: {e}", "capture flush failed: {e}"],
  ["Nie udało się zapisać końcowego fragmentu nagrania: {e}", "final flush failed: {e}"],
  ["Zdarzenie nagrania przekracza limit rozmiaru", "capture event exceeds bounded record size"],
  ["Przekroczony limit kolejki nagrania; brak części wejść", "capture queue budget exceeded; input gap"],
  ["Kolejka nagrania niedostępna; brak części wejść", "capture queue unavailable; input gap"],
  ["Przekroczony limit plików sesji; nagrywanie zatrzymane", "run disk budget exceeded; recording stopped"],
  ["Przekroczony łączny limit nagrań; początki aktywnych sesji zachowano", "total recording disk budget exceeded; active origins preserved"],
  ["Nie udało się zakodować zdarzenia nagrania", "capture serialization failed"],
  ["Zdarzenie po zamknięciu nagrania", "event after capture closed"],
  ["Ta wersja odtwarzania nie obsługuje stanu decyzyjnego AI/EA", "unsupported AI/EA decision-state contract"],
  ...
  ["nieznany wynik {cmd}: retcode={code}; wymagane potwierdzenie wykonania przed nowymi wejściami", "unknown outcome of {cmd}: retcode={code}; execution confirmation required before new entries"],
  ["nieznany wynik {cmd}: timeout po wysłaniu; odczyt stanu nie zastępuje potwierdzenia wykonania", "unknown outcome of {cmd}: timeout after dispatch; a state read does not replace execution confirmation"],
  ["nieznany wynik {cmd}: {e}; wymagane potwierdzenie wykonania przed nowymi wejściami", "unknown outcome of {cmd}: {e}; execution confirmation required before new entries"],
  ...
  ["Podsumowanie strategii: {dzis:+.2} $ dzisiaj", "Strategy summary: {dzis:+.2} $ today"],
  ...
  ["Dzisiaj · wynik strategii {0} $ · obsunięcie dnia {1} $ · transakcji {2}", "Today · strategy result {0} $ · daily drawdown {1} $ · trades {2}"],
  ...
  ["Skuteczność strategii od startu · {0} z {1} ({2} %) · profit factor {3}", "Strategy win rate since startup · {0} of {1} ({2} %) · profit factor {3}"],
  ...
  ["`equity_floor_pct = {}` blokuje nowe wejścia sprawdzane przez bramkę rachunku, gdy equity jest równe lub niższe od tego % kapitału startowego. Nie anuluje wcześniej wystawionych zleceń oczekujących ani nie zamyka pozycji; zlecenia te nadal mogą się wykonać. Po wzroście equity ponad próg wejścia mogą wznowić się samoczynnie, jeśli inne reguły na to pozwalają. To nie limit maksymalnej straty.", "`equity_floor_pct = {}` blocks new entries checked by the account entry gate when equity is equal to or below this % of starting capital. It does not cancel pending orders already submitted or close positions; those orders can still fill. If equity rises above the threshold, entries may resume automatically if other rules allow them. This is not a maximum loss limit."],
  ...
  ["EDYCJA WEJŚCIA: pierwszy pełny chroniony sygnał oceniony w chwili odbioru", "ENTRY EDIT: first complete protected source evaluated at receive time"],
  ["ŹRÓDŁO WEJŚCIA: anulowanie przez nadawcę blokuje ponowną aktywację", "ENTRY SOURCE: publisher withdrawal prevents reactivation"],
  ["ŹRÓDŁO WEJŚCIA: spóźniony NEW nie może zastąpić wcześniej odebranej edycji wejścia", "ENTRY SOURCE: late NEW cannot replace an already received entry edit"],
  ["ŹRÓDŁO WEJŚCIA: powiązany CANCEL zapisany przed jego sygnałem wejścia; nie wybrano obcego koszyka", "ENTRY SOURCE: bound CANCEL saved before its source entry; no unrelated basket selected"],
  ["EDYCJA WEJŚCIA: źródło {orig} nie ma koszyka; przyjęcie wymaga zgody presetu i pełnego chronionego wejścia", "ENTRY EDIT: source {orig} has no basket; recovery requires the preset policy and a complete protected entry"],
  ["DOKŁADKA TEMPOWA: TP {tp:.2} jest już za rynkiem; zlecenie nie zostało wysłane, slot pozostaje dostępny", "FAST ADDON: TP {tp:.2} is beyond the market; no order sent and capacity remains available"],
  ...
  ["KRZYWA LOTA — BLOKADA: {reason:?}; nowe zlecenie wstrzymane", "LOT GROWTH HOLD: {reason:?}; new order withheld"],
  ["KONTEKST LOTA — BLOKADA: {reason:?}; nowe zlecenie wstrzymane", "LOT CONTEXT HOLD: {reason:?}; new order withheld"],
  ["KONTEKST LOTA: obserwacja wielkości pozycji przed wysłaniem zlecenia", "LOT CONTEXT: causal pre-send sizing observation"],
  ["Ustawienia krzywej lota: nieznane pole {key}", "Lot growth settings: unknown field {key}"],
  ["Ustawienia krzywej lota: nieprawidłowe typy pól lub wariant", "Lot growth settings: invalid field types or enum"],
  ["Ustawienia krzywej lota: wartości poza dozwolonym zakresem", "Lot growth settings: values outside the allowed range"],
  ["Ustawienia krzywej lota: siła tłumienia poza zakresem [0, 2]", "Lot growth settings: attenuation strength outside [0, 2]"],
